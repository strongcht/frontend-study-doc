...
| 特性 | Set | Array | 
| --- | --- | --- | 
| 唯一性 | 是，不允许重复值 | 否，可以包含重复值 | 
| 数据结构 | 哈希表 | 动态数组 | 
| 查找时间复杂度 | O(1) | O(1) | 
| 添加/删除时间复杂度 | O(1) | 平均O(1)，最坏O(n) | 
| 方法 | add, delete, has, clear等 | push, pop, shift, unshift, splice, slice等 | 
| 迭代 | for...of, forEach, 扩展运算符 ... | for...of, forEach, 扩展运算符 ..., for, map, filter, reduce等 | 
| 转换为Array | Array.from, 扩展运算符 ... | - | 
| 转换为Set | new Set | - |
...
可以看到修改原型的时候p的构造函数不是指向Person了，因为直接给Person的原型对象直接用对象赋值时，它的构造函数指向的了根构造函数Object，所以这时候`p.constructor === Object` ，而不是`p.constructor === Person`。要想成立，就要用`constructor`指回来：
...
![原型链终点](images/20240327-js-3.png)
...
Promise有五个常用的方法：then()、catch()、all()、race()、any()、finally。下面就来看一下这些方法。
...
all方法可以完成并行任务， 它接收一个可迭代对象，数组的每一项都是一个promise对象。当数组中所有的promise的状态都达到resolved的时候，all方法的状态就会变成resolved，如果有一个状态变成了rejected，那么all方法的状态就会变成rejected。
```js
...
// 实现 promise.all 方法  接收的是一个可迭代对象 Array、String等
...
## 7.  对async/await 的理解
async/await其实是Generator 的语法糖，它能实现的效果都能用then链来实现，它是为优化then链而开发出来的。从字面上来看，async是“异步”的简写，await则为等待，所以很好理解async 用于声明一个 function 是异步的，而 await 用于等待一个异步方法执行完成。当然语法上强制规定await只能出现在asnyc函数中，先来看看async函数返回了什么：
```js
...
浏览器通常使用的垃圾回收方法：`标记清除`，`引用计数`, `分代垃圾回收`。
...
现代浏览器的垃圾回收机制主要负责管理和回收应用程序中的无用内存，以避免内存泄漏和优化性能。不同的浏览器使用不同的引擎，如Chrome使用V8、Firefox使用SpiderMonkey、Safari使用JavaScriptCore等，但它们在垃圾回收的基本原理和方式上有许多共同点。下面详细介绍这些机制的原理和方式。


**3）分代垃圾回收（Generational Garbage Collection）**

> 分代垃圾回收将堆内存划分为不同的代（generations），通常包括新生代和老生代。

###### 新生代（Young Generation）
- **特点**：包含生命周期较短的对象。
- **回收算法**：Scavenge算法，基于复制的回收（Copying Collection）。
  - **分配区（Nursery）**：新生代对象分配在两个半空间（From空间和To空间）中。
  - **复制回收**：当From空间满时，扫描所有存活对象并将其复制到To空间，丢弃不可达对象。然后交换From和To空间的角色。
  - **晋升（Promotion）**：经过多次复制后仍存活的对象被晋升到老生代。

###### 老生代（Old Generation）
- **特点**：包含生命周期较长的对象。
- **回收算法**：标记-清除（Mark-and-Sweep）和标记-压缩（Mark-and-Compact）。
  - **标记阶段**：遍历所有可达对象并标记它们。
  - **清除阶段**：回收未标记的对象，释放其内存。
  - **压缩阶段**：在必要时移动存活对象以消除内存碎片。

**4） 增量回收和并发回收**

> 为了减少垃圾回收对应用程序执行的影响，现代垃圾回收机制采用了增量和并发回收技术。

+ 增量回收（Incremental Garbage Collection）
  - **特点**：将垃圾回收过程分解成多个小步骤，分散在程序执行过程中。
  - **优势**：减少每次垃圾回收引起的暂停时间。

+ 并发回收（Concurrent Garbage Collection）
  - **特点**：在独立线程中进行垃圾回收，与应用程序并发执行。
  - **优势**：进一步减少暂停时间，提高应用程序的响应速度。
...
垃圾回收通常在以下情况下触发：
- 新生代空间填满时触发Scavenge。
- 老生代达到一定使用率时触发标记-清除或标记-压缩。
- 某些特定条件下（如内存使用率高、手动调用gc()）可能触发垃圾回收。
