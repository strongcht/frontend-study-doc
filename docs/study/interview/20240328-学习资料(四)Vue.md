...
![双向数据绑定](images/20240328-vue-2.png)
...
3. 需要缓存，判断他当前是否在缓存数组里面：
  - 存在，则将他原来位置上的 key 给移除，同时将这个组件的 key 放到数组最后面（LRU）(least recently used)
...
![vue单页面与多页面区别](images/20240328-vue-5.png)
...
![minx和extend](images/20240328-vue-6.png)
...
![vuex](images/20240328-vue-8.png)
...
**（2）各模块在核心流程中的主要功能：**
 ...
 + actions∶ 操作行为处理模块。负责处理Vue Components接收到的所有交互行为。包含同步/异步操作，支持多个同名方法，按照注册的顺序依次触发。向后台API请求的操作就在这个模块中进行，包括触发其 他action以及提交mutation的操作。该模块提供了Promise的封装，以支持action的链式触发。
 + commit∶状态改变提交操作方法。对mutation进行提交，是唯一能执行mutation的方法。
 + mutations∶状态改变操作方法。是Vuex修改state的唯一推荐方法，其他修改方式在严格模式下将会报错。该方法只能进行同步操作，且方法名只能全局唯一。操作之中会有一些hook暴露出来，以进行state 的监控等。
 + state∶ 页面状态管理容器对象。集中存储Vuecomponents中data对象的零散数据，全局唯一，以进行统一的状态管理。页面显示所需的数据从该对象中进行读取，利用Vue的细粒度数据响应机制来进行高效的 状态更新。
...
4. 去除Vuex时的mutations和modules ，只有state、getters和actions。
...
#### 3.state修改方式
> 通常state 是store的核心，state被定义为一个返回初始状态的函数。 可以通过不同的方法修改state中的值。

1. 直接修改
state是 容许直接修改值的。App.vue中通过直接修改current 的值。
```vue
...
```
2. 通过调用 $patch方法来实现用一个 state的补丁对象在同一时间更改多个属性，也就是批量修改。
```js
const change = ()=> {
  Test.$patch({
    current: 100000,
...
3. 通过$patch方法接受一个函数来实现，此方法可自定义修改逻辑，常用。
```js
const change = ()=> {
  Test.$patch((state)=> {
    state.name = '小海学 Pinia'
    state.current = 6
  })
}
```
4. 通过完全替换store的 state，也就是$state 来实现。

注意：此方法不支持 不完全替换store的state，官方说会破坏响应式。
```js
const change = ()=> {
  Test.$state = {
    name: 'new name',
...
export const useTestStore = defineStore(Names.TEST, {
  ...
  actions: {
    setCurrent(num: number = 9999) {
      this.current = num
    }
  }
})
```
在App.vue中调用store 中的actions 中的 setCurrent方法来实现。
```vue
<script setup lang="ts">
import {useTestStore} from '@/stores';
...
自定义使用 localStorage 或 sessionStorage 实现持久化

[插件](https://prazdevs.github.io/pinia-plugin-persistedstate/zh/guide/)
1. 安装
```sh
...
[vue虚拟列表 vue-virtual-scroller](https://github.com/Akryum/vue-virtual-scroller)