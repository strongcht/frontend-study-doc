...
then接收 非'function' 则为 x => x 所以 then(2) 和 then(Promise.resolve(3)) 相当于 接收上个传入的值；
...
+ 最后console.log(p)打印出`Promise{<resolved>: 1}`；
...
（3）第三轮事件循环开始，此时宏任务 Event Queue 中只剩 `setTimeout2` 了，执行该宏任务。在此之前，会先清空微任务 Event Queue 中的所有任务，即先执行 `process3` 和 `then3`
